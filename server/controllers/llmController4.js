...
function cleanJSONString(str) {
  return str
    ...
    .replace(/\\(\r?\n)/g, "")
    .replace(/\\\s+/g, " ")
    // Fix invalid JSON escape sequences (e.g. \l from </li> → l, \c, \p, etc.)
...
async function optimizeFeedbackForAI(webUrl) {
  try {
    ...
    for (const [title, key] of Object.entries(KEY_MAP)) {
      const group = feedbacks.filter((f) => f.title === title);
      if (group.length === 0) continue;
      const openItems = group.filter((f) => !f.status);
      const resolvedItems = group.filter((f) => f.status);
      ...
      categories[key] = {
        ...
        d_open: openItems.slice(0, 20).map(toEntry).filter((e) => e.desc),
        d_resolved: resolvedItems.slice(0, 20).map(toEntry).filter((e) => e.desc),
      };
    }

    return { [webUrl]: { stats, ...categories } };
  } catch (error) {
  ...
  try {
    ...
    for (const [site, siteFeedbacks] of Object.entries(bySite)) {
      ...
      const stats = {
        ...
        avgSev: (siteFeedbacks.reduce((s, f) => s + (f.severity || 0), 0) / total).toFixed(1),
      };

      const categories = {};
      for (const [title, key] of Object.entries(KEY_MAP)) {
        const group = siteFeedbacks.filter(f => f.title === title);
        if (group.length === 0) continue;
        const openItems = group.filter(f => !f.status);
        const resolvedItems = group.filter(f => f.status);
        const n = group.length;
  ...
  try {
    ...
    if (intent.needsAllData) {
      ...
      for (const site of availableSites) {
        if (site) Object.assign(optimizedData, await optimizeFeedbackForAI(site));
      }
